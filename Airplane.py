...
from settings import RES_X, RES_Y
...
class Airplane:
    ...
    def set_state(self, event_key):
        ...
        keys = [self.key_left, self.key_right, self.key_up, self.key_down,
                self.key_fire]  # Список функциональных клавиш
    ...
    def turning(self, dt, change_speed_vector):  # поворачиваем объект
        ...
        self.speed_vector = (
            change_speed_vector[0] / dt * 1000, change_speed_vector[1] / dt * 1000)  # воссанавливаем вектор скорости
    ...
    def parallel_motion(self):
        if self.position_vector[0] < 0:  # если объект заходит за границы слева то
            ...
            self.position_vector = (
                self.position_vector[0] + RES_X,  # вектор позициии выходит с правого края
            ...
            self.position_vector = (self.position_vector[0],  # вектор позициии выходит с правого края
                                    self.position_vector[1] + RES_Y)
        elif self.position_vector[0] > RES_X:  # если объект заходит за границы справа то
            self.status_of_parallel_motion = True  # cтатус становится True
            self.position_vector_2 = self.position_vector  # вектор параллельного движения равен вектору позиции
            self.position_vector = (
                self.position_vector[0] - RES_X,  # вектор позициии выходит с левого края
                self.position_vector[1])
        elif self.position_vector[1] > RES_Y:
            self.status_of_parallel_motion = True  # cтатус становится True
            self.position_vector_2 = self.position_vector  # вектор позиции параллельного движения равен вектору позиции
            self.position_vector = (self.position_vector[0],  # вектор позициии выходит с правого края
                                    self.position_vector[1] - RES_Y)
    ...
    def accel_speed(self, dt, change_speed_vector):
        ...
        Ускоряем объект
        """
        width_vector = math.sqrt(
            change_speed_vector[0] ** 2 + change_speed_vector[1] ** 2)  # высчитываем длинну вектора скорости

        normal_vector = (change_speed_vector[0] / width_vector, change_speed_vector[1] / width_vector)

        if self.dir_of_accel:
            self.accel_vector = (
                (change_speed_vector[0] / width_vector * self.d_acsel) / 1000 * dt,  # рассчитываем вектор ускорения
        ...
        if self.dir_of_accel == 'up' and width_vector < 6 * math.sqrt(
                                normal_vector[0] ** 2 + normal_vector[1] ** 2):  # если направление ускорения вверх
            change_speed_vector = (change_speed_vector[0] + self.accel_vector[0],
                                   change_speed_vector[1] + self.accel_vector[
                                       1])  # прибовляем к ветору скорости вектор ускорения

        elif self.dir_of_accel == 'down' and width_vector > math.sqrt(
                                normal_vector[0] ** 2 + normal_vector[1] ** 2):  # если направление ускорения вниз
            change_speed_vector = (change_speed_vector[0] - self.accel_vector[0],
                                   change_speed_vector[1] - self.accel_vector[
                                       1])  # вычитаем к ветору скорости вектор ускорения
    ...
    def gravity(self, dt, change_speed_vector):  # измениям вектор скорости с учетом гравитации
        ...
        change_speed_vector = (
            change_speed_vector[0] - gravity_vector[0],  # высчитываем ветор скорости независимый о фпс
    ...
    def fire(self):  # добавляем в список пуль 1 пулю
        self.bullets.append(
            Bullet((self.position_vector[0] - 25, self.position_vector[1] - 25), self.speed_vector,
                                    num_airplane=self.num_img))
